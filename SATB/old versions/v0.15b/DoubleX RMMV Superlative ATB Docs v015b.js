 ...
 *         configurations
 *      3. If you want to keep the current parameter values in the plugin
 ...
 *      4. If you wish to use DoubleX RMMV Superlative ATB Unit Test, place it
 ...
 *      5. Lets players forces actor charge by long pressing the actor sprite
 *      6. Lets you set some skills to demand a set period to charge up before
 *         they become usable actions that can be inputted by battlers
 *      7. Lets you set some skills to demand a set period to cool down before
 *         they become usable actions that can be inputted by battlers again
 *      8. Adds a parameter for each sprite/window class to be called per
 *         frame so you can control which parameter cache to be
 *         enabled/disabled
 *      9. Fixes the actor command window not selecting the last command when
 *         it becomes able to be shown again bug
 *      10. Fixes the autobattle actor freezed charge/cooldown with the Delay
 *          module enabled
 *      11. Fixes the compatibility issues/bugs when the CTB Module interacts
 *          with SEK_ChangeActor
 *      12. Adds the Yanfly Engine Plugins - Battle Engine Core action
 *          sequences for the delay module
 *============================================================================*/
/*:
 ...
 *           be ignored and the common event with id 1 to be run when common
 *           events can be run
 *      4. didStartATBFill suffix: entry
 *         - Sets additional events to happen right after the battler involved
 ...
 *           be ignored and the common event with id 1 to be run when common
 *           events can be run
 *      5. willCancelCharge suffix: entry
 ...
 *           parameter to be ignored and the common event with id 1 to be run
 *           when common events can be run
 *      6. didStartForceCharge suffix: entry
 *         - Sets additional events to happen right after the battler involved
 ...
 *      7. willCancelCooldown suffix: entry
 *         - Sets additional events to happen right before the battler
 *           involved cancels the ATB charge
 *         - suffix can be cfg, event or script
 *         - (Advanced)Please refer to Did Finish Input Functions in the event
 ...
 *           common event with id 1 to be run when common events can be run
 *      8. didCoreATBBecomeFull suffix: entry
 ...
 *           be run
 *      9. didCoreATBBecomeNotFull suffix: entry
 *         - Sets additional events to happen right after the ATB value of the
 *           battler involved becomes not full
 *         - suffix can be cfg, event or script
 *         - (Advanced)Please refer to Did Finish Input Functions in the event
 *           module of the configuration plugin for using cfg or script
 *           suffixes, or the eval variant
 ...
 *           cause the default didCoreATBBecomeNotFull parameter to be ignored
 *           and the common event with id 1 to be run when common events can
 *           be run
 *      10. didChargeATBBecomeNotFull suffix: entry
 ...
 *            and the common event with id 1 to be run when common events can
 *            be run
 *      11. didAddInputableActor suffix: entry
 ...
 *          - (Advanced)It's supposed to return a natural number
 *          - E.g.:
 ...
 *           using this script call
 *         - E.g.:
 *           $gameTroop.aliveMembers()[1].setDelaySecCounter(0.5) will set the
 *           delay counter locking the action inputs of the 1st alive troop
 *           member as 0.5 seconds
 *      2. addDelaySecCounter(increment)
 *         - The same as the script call setDelaySecCounter(delay) except that
 *           this one adds the delay counter instead
 *      3. multiplyDelaySecCounter(multiplier)
 *         - The same as the script call setDelaySecCounter(delay) except
 *           that this multiplies the delay counter instead
 *      4. delaySecCounter()
 *         - Returns the delay counter locking the action inputs of the
 *           battler involved
 *         - E.g.:
 *           $gameTroop.aliveMembers()[1].delaySecCounter() will return the
 *           delay counter locking the action inputs of the 1st alive troop
 *           member
 *      (v0.05a+)Rate Module:
 *      1. coreSATBRate()
...
DoubleX_RMMV["Superlative ATB Documentations"] = "v0.15b";
//

// Ensures that all plugins are of the latest version
DoubleX_RMMV.SATB_VERS = {
    Parameters: "v0.15b",
    Configurations: "v0.15a",
    Implementations: "v0.15b",
    "Unit Tests": "v0.15b",
    Compatibilities: "v0.04a",
