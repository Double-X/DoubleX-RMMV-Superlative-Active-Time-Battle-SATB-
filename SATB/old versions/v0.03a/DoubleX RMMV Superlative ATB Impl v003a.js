...
DoubleX_RMMV["Superlative ATB Implementations"] = "v0.03a";
//
...
//
function Window_SATBBar() {
    "use strict";
    this.initialize.apply(this, arguments);
} // Window_SATBBar(v0.03a+)
...
(function(SATB) {
    ...
    $.initialize = function(battler) {
        this._battler = battler, this._coreATB = this._lastCoreATB = 0;
     ...
     */
    $.coreMax = function() { return this._battler.satbNoteResult_("coreMax"); };
...
(function(SATB) {
    ...
    $.standardPadding = function() {
        return 0; // The whole window is just the battler ATB bar
    }; // $.standardPadding
     ...
     */
    $.update = function() {
        if (!BattleManager.isSATBModuleEnabled("IsBarEnabled")) {
     ...
     */
    $._isVisible = function() {
        return $gameSystem.satbParamFunc("isShowATBBar").call(this._battler);
    }; // $._isVisible
    ...
    $._update = function() {
        ...
        //
        if (!isUpdateBar && !isUpdatePadding && !isUpdateTextPadding &&
                !isUpdateSize && !isUpdateTextColor && !isUpdateTextXY &&
                !isUpdateText) return;
    ...
    $._isUpdateBar = function() {
        // All of them must be run per frame to keep all these caches up to date
        var isUpdateWHProportion = this._isUpdateWHProportion();
        var isUpdateBarColors = this._isUpdateBarColors();
        //
        return isUpdateWHProportion || isUpdateBarColors;
    }; // $._isUpdateWHProportion
     ...
     * @returns {Boolean} The check result
     */
    $._isUpdateWHProportion = function() {
        // All of them must be run per frame to keep all these caches up to date
        var isUpdateW = this._isCacheUpdated("_barW", this._w());
        var isUpdateH = this._isCacheUpdated("_barH", this._h());
        var isUpdateProportion =
                this._isCacheUpdated("_proportion", this._fillProportion());
        //
        return isUpdateW || isUpdateH || isUpdateProportion;
    }; // $._isUpdateWHProportion
    ...
    $._isCacheUpdated = function(cache, newVal) {
        var oldVal = this[cache];
        this[cache] = newVal;
        return oldVal !== this[cache];
    }; // $._isCacheUpdated
...
(function(SATB) {
     ...
     */
    $.update = function() {
        if (!BattleManager.isSATBModuleEnabled(this._moduleParam)) {
            return this.visible = false;
        }
        $$.update.call(this);
        this._updateAll();
     ...
     */
    $._x = function() {
        return $gameSystem.satbParamFunc(this._winXParam)();
    }; // $._x
     ...
     */
    $._y = function() {
        return $gameSystem.satbParamFunc(this._winYParam)();
    }; // $._w
     ...
     */
    $._w = function() {
        return $gameSystem.satbParamFunc(this._winWParam)();
    }; // $._w
     ...
     */
    $._h = function() {
        return $gameSystem.satbParamFunc(this._winHParam)();
    }; // $._h
    ...
    $._updateText = function() {
        ...
        var isUpdateText = this._isCacheUpdated("_text", this._formattedText());
        //
        if (!isUpdatePadding && !isUpdateTextPadding && !isUpdateSize &&
                !isUpdateXY && !isUpdateText) return;
        if (isUpdatePadding) this.updatePadding();
        if (isUpdateSize) this.resetFontSettings();
        this._redrawText();
    }; // $._updateText
     ...
     */
    $._textXOffset = function() {
        return $gameSystem.satbParamFunc(this._textXParam)();
    }; // $._textXOffset
     ...
     */
    $._textYOffset = function() {
        return $gameSystem.satbParamFunc(this._textYParam)();
    }; // $._textYOffset
...
(function(SATB) {
     ...
     */
    $.onForceRun = function() {
        if (this.isForceRun()) return SoundManager.playBuzzer();
    ...
    $.onForceStop = function() {
        if (this.isForceStop()) return SoundManager.playBuzzer();
...
(function(SATB) {
    ...
    $.isRunSATBFrameUpdate = function() {
        if (!BattleManager.isSATBModuleEnabled("IsWaitEnabled")) return true;
    ...
    _SATB._createForceRunCmdWin = function() {
        this._satb.forceRunCmdWin = new Window_SATBForceRunCmd(
                _SATB._onTouchForceRunCmdWin.bind(this));
     ...
     * @since v0.03a @version v0.03a
     */
    _SATB._onTouchForceRunCmdWin = function() {
        var forceStatusWin = this._satb.forceStatusWin;
        forceStatusWin.onForceRun();
        this._satb.forceStopCmdWin.changePaintOpacity(true);
        if (!forceStatusWin.isForceRun()) return;
        this._satb.forceRunCmdWin.changePaintOpacity(false);
    }; // _SATB._onTouchForceRunCmdWin
    ...
    _SATB._createForceStopCmdWin = function() {
        this._satb.forceStopCmdWin = new Window_SATBForceStopCmd(
                _SATB._onTouchForceStopCmdWin.bind(this));
     ...
     * @since v0.03a @version v0.03a
     */
    _SATB._onTouchForceStopCmdWin = function() {
        var forceStatusWin = this._satb.forceStatusWin;
        forceStatusWin.onForceStop();
        this._satb.forceRunCmdWin.changePaintOpacity(true);
        if (!forceStatusWin.onForceStop()) return;
        this._satb.forceStopCmdWin.changePaintOpacity(false);
    }; // _SATB._onTouchForceStopCmdWin
