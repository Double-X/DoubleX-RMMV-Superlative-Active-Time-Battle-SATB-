...
DoubleX_RMMV["Superlative ATB Configurations"] = "v0.16a";
//
...
(function(SATB) {
    ...
    SATB.params = {
         ...
         *    - Lets you set the action cost of some skills/items
         *--------------------------------------------------------------------*/
        act: {

            /**
             * THIS FUNCTION SHOULD BE PERFORMANT ENOUGH TO BE RUN PER FRAME
             * Sets whether the Action Module will be enabled
             * Hotspot/Nullipotent
             * @since v0.16a @version v0.16a
             * @returns {Boolean} The check result
             */
            IsActEnabled: function() {
                return false; // Always disables the Action Module
            }, // IsActEnabled

            /**
             * Sets the default action mode when ther are no action mode notetag
             * Hotspot/Nullipotent
             * @since v0.16a @version v0.16a
             * @returns @enum {String} bundle/batch/discrete/continuous
             */
            actMode: function() {
                return "batch"; // Always uses the batch action mode
            } // actMode
        ...
        /*--------------------------------------------------------------------
         *    (v0.15a+)Delay Module
         *    - Lets you sets the delay counter locking battler action inputs
         *--------------------------------------------------------------------*/
        delay: {

            /**
             * Sets whether the Delay Module will be enabled
             * Nullipotent
             * @since v0.15a @version v0.15a
             * @returns {Boolean} The check result
             */
            IsDelayEnabled: function() {
                return false; // Always disables the Event Module
            }, // IsDelayEnabled

            /**
             * The this pointer refers to the battler involved
             * Sets the delay counter locking action inputs of the battler
             * Nullipotent
             * @since v0.15a @version v0.15a
             * @returns {Nonnegative Num} The delay counter locking action input
             */
            delaySecs: function() {
                // Returns 0.5 if it's enemy/autobattle/confused and 0 otherwise
                if (this.isEnemy()) return 0.5;
        ...
        event: {
            ...
            /**
             * The this pointer refers to the battler involved
             * Sets what extra to happen right after setting max no of act slots
             * @since v0.16a @version v0.16a
             */
            didSetMaxActTimes: function() { /* Does nothing extra */ },
            ...
            /**
             * The this pointer refers to the battler involved
             * Sets what extra to happen right after battler delay counter ends
             * @since v0.13a @version v0.13a
             */
            didDelayCounterEnd: function(stateId) { /* Does nothing extra */ }
    ...
    SATB.notes = {
        ...
        /*--------------------------------------------------------------------
         *    (v0.16a+)ATB Action Cost Functions
         *    - Setups ATBACX used by this plugin's actCost notetags
         *--------------------------------------------------------------------*/
        /* ATBACX names can only use alphanumeric characters
         * The 1st character of ATBACX can't be a number
         * The below ATBACX are examples added to help you set your ATBACX
         * You can freely use, rewrite and/or delete these examples
         * (Advanced)ATBACX can be used on notetags other than actCost if you
         * really know what you're truly doing
         * If you're to use them that way, you must at least ensure following:
         * 1. The function contexts are the same across different notetags
         * 2. The function argument lists(with the paraemter names being exactly
         *    the same) are the same across different notetags
         * 3. The function return types are the same across different notetags
         * (Advanced)You're encouraged and recommended to write modular ATBACX
         */
        actCost: {
             ...
             * @enum @param {DatumType} datumType - Refers to reference tag
             *                                      NOTE_DATA_TYPES
             * @param {Num} latestActCost - Action cost value up to latest note
             *                              DON'T USE IT IF THE CHAINING RULE IS
             *                              AN ASSOCIATIVE OPERATOR
             *                              Refer to reference tag
             *                              ASSOCIATIVE_CHAINING_RULE
             * @returns {Num} The action cost value for the battler involved
             */
            ATBAC_1: function(datum, datumType, latestActCost) {
             ...
             * @returns {Num} The action cost value for the battler involved
             */
            ATBAC_AGI: function(datum, datumType, latestActCost) {
                // The current action cost * 999 / the battler's AGI
                return latestActCost * 999 / this.agi;
                //
            }, // ATBAC_AGI
             ...
             * @returns {Num} The action cost value for the battler involved
             */
            ATBAC_VAR: function(datum, datumType, latestActCost) {
                // Returns the Number value in the game variable with id 4
                return +$gameVariables.value(4);
                // Numbers with decimals should be stored in the String form
            }, // ATBAC_VAR

            // Adds new ATBACX here


        }, // actCost

        /*--------------------------------------------------------------------
         *    (v0.16a+)ATB Action Mode Functions
         *    - Setups ATBAMX used by this plugin's actCost notetags
         *--------------------------------------------------------------------*/
        /* ATBAMX names can only use alphanumeric characters
         * The 1st character of ATBAMX can't be a number
         * The below ATBAMX are examples added to help you set your ATBAMX
         * You can freely use, rewrite and/or delete these examples
         * (Advanced)ATBAMX can be used on notetags other than actCost if you
         * really know what you're truly doing
         * If you're to use them that way, you must at least ensure following:
         * 1. The function contexts are the same across different notetags
         * 2. The function argument lists(with the paraemter names being exactly
         *    the same) are the same across different notetags
         * 3. The function return types are the same across different notetags
         * (Advanced)You're encouraged and recommended to write modular ATBAMX
         */
        actMode: {
             ...
             * @returns @enum {String} bundle/batch/discrete/continuous
             */
            ATBAM_BUNDLE: function(datum, datumType) {
                return "bundle"; // Always returns bundle
            }, // ATBAM_BUNDLE
             ...
             * @returns @enum {String} bundle/batch/discrete/continuous
             */
            ATBAM_DISCRETE: function(datum, datumType) {
                return "discrete"; // Always returns bundle
            }, // ATBAM_DISCRETE
             ...
             * @returns @enum {String} bundle/batch/discrete/continuous
             */
            ATBAM_CONTINUOUS: function(datum, datumType) {
                return "continuous"; // Always returns continuous
            }, // ATBAM_VAR

            // Adds new ATBAMX here
         ...
         *    the same) are the same across different notetags
         * 3. The function return types are the same across different notetags
         * (Advanced)You're encouraged and recommended to write modular CHMATBX
         */
        chargeMax: {
        ...
        countdown: {
             ...
             * really know what you're truly doing
             * If you're to use them that way, you must at least ensure the
             * following:
             * 1. The function contexts are the same across different notetags
        ...
        delay: {
            ...
            /*----------------------------------------------------------------
             *    (v0.15a+)Delay Condition Functions
             *    - Setups DCX used by plugin's delay notetags
             *----------------------------------------------------------------*/
            /* DCX names can only use alphanumeric characters
             * The 1st character of DCX can't be a number
             * The below DCX are examples added to help you set your DCX
             * You can freely use, rewrite and/or delete these examples
             * (Advanced)DCX can be used on notetags other than delay if you
             * really know what you're truly doing
             * If you're to use them that way, you must at least ensure the
             * following:
             * 1. The function contexts are the same across different notetags
             ...
             * 3. The function return types are the same across different
             *    notetags
             * (Advanced)You're encouraged and recommended to write modular DCX
             */
             ...
             * @returns {Boolean} The check result
             */
            DC_TRUE: function(datum, datumType) {
                return true; // Always sets the state as an delay state
            }, // DC_TRUE
             ...
             * @returns {Boolean} The check result
             */
            DC_BATTLER_NAME: function(datum, datumType) {
                // Sets the state as delay state if the battler name is Test
                return this.name() === "Test";
                //
            }, // DC_BATTLER_NAME
             ...
             * @returns {Boolean} The check result
             */
            DC_SWITCH: function(datum, datumType) {
                // Sets the state as delay state if switch with id x is on
                return $gameSwitches.value(x);
                //
            }, // DC_SWITCH
            ...
            /*----------------------------------------------------------------
             *    (v0.15a+)Delay Counter Functions
             *    - Setups DCUX used by plugin's delay notetags
             *----------------------------------------------------------------*/
            /* DCUX names can only use alphanumeric characters
             * The 1st character of DCUX can't be a number
             * The below DCUX are examples added to help you set your DCUX
             * You can freely use, rewrite and/or delete these examples
             * (Advanced)DCUX can be used on notetags other than delay if you
             * really know what you're truly doing
             ...
             * 3. The function return types are the same across different
             *    notetags
             * (Advanced)You're encouraged and recommended to write modular DCUX
             */
             ...
             * @enum @param {DatumType} datumType - Refers to reference tag
             *                                      NOTE_DATA_TYPES
             * @returns {Number} The delay interval
             */
            DCU_1: function(datum, datumType) {
                return 1; // Always set the interval as 1 delay unit
            }, // CU_FRAME
             ...
             * @returns {Number} The delay interval
             */
            DCU_STATE: function(datum, datumType) {
                // Returns 2 and 1 if the battler's affected with state of id x
                return this.isStateAffected(x) ? 2 : 1;
                //
            }, // DCU_STATE
             ...
             * @returns {Number} The delay interval
             */
            DCU_VAR: function(datum, datumType) {
                // Returns the value in the game variable with id x
                return $gameVariables.value(x);
                //
            }, // DCU_VAR
        ...
        /*--------------------------------------------------------------------
         *    (v0.06a+)Did Set Max Act Times Functions
         *    - Setups DSMATX used by plugin's didSetMaxActTimes notetags
         *--------------------------------------------------------------------*/
        /* DSMATX names can only use alphanumeric characters
         * The 1st character of DSMATX can't be a number
         * The below DSMATX are examples added to help you set your DSMATX
         * You can freely use, rewrite and/or delete these examples
         * (Advanced)DSMATX can be used on notetags other than didSetMaxActTimes
         * if you really know what you're truly doing
         * If you're to use them that way, you must at least ensure following:
         * 1. The function contexts are the same across different notetags
         * 2. The function argument lists(with the paraemter names being exactly
         *    the same) are the same across different notetags
         * 3. The function return types are the same across different notetags
         * (Advanced)You're encouraged and recommended to write modular DSMATX
         */
        didSetMaxActTimes: {
             ...
             *                                      NOTE_DATA_TYPES
             */
            DSMAT_COMMON_EVENT: function(datum, datumType) {
                // Runs the common event with id x when common events can be run
                $gameTemp.reserveCommonEvent(x);
                //
            }, // DSMAT_COMMON_EVENT
             ...
             *                                      NOTE_DATA_TYPES
             */
            DSMAT_SWITCH: function(datum, datumType) {
                // Sets the game switch with id x to be on
                $gameSwitches.setValue(x, true);
                //
            }, // DSMAT_SWITCH
             ...
             *                                      NOTE_DATA_TYPES
             */
            DSMAT_VAR: function(datum, datumType) {
                // Sets the value of the game variable with id x to be y
                $gameVariables.setValue(x, y);
                //
            }, // DSMAT_VAR

            // Adds new DSMATX here
        ...
        didDecreaseCountdownStateTurn: {
            ...
            DDCST_POISON: function(datum, datumType, stateId) {
                ...
                this._result.success = this._result.hpAffected = true;
                var hpDamage = this.mhp * 0.02;
                this.gainHp(-hpDamage);
                this.onDamage(hpDamage);
        ...
        /*--------------------------------------------------------------------
         *    (v0.13a+)Did Delay Counter End Functions
         *    - Setups DDCEX used by didDelayCounterEnd notetags
         *--------------------------------------------------------------------*/
        /* DDCEX names can only use alphanumeric characters
         * The 1st character of DDCEX can't be a number
         * The below DDCEX are examples added to help you set your DDCEX
         * You can freely use, rewrite and/or delete these examples
         * (Advanced)DDCEX can be used on notetags other than
         * didDelayCounterEnd if you really know what you're truly doing
         * If you're to use them that way, you must at least ensure following:
         * 1. The function contexts are the same across different notetags
         * 2. The function argument lists(with the paraemter names being exactly
         *    the same) are the same across different notetags
         * 3. The function return types are the same across different notetags
         * (Advanced)You're encouraged and recommended to write modular DDCEX
         */
        didDelayCounterEnd: {
             ...
             *                                      NOTE_DATA_TYPES
             */
            DDCE_COMMON_EVENT: function(datum, datumType) {
                // Runs the common event with id x when common events can be run
                $gameTemp.reserveCommonEvent(x);
                //
            }, // DDCE_COMMON_EVENT
             ...
             *                                      NOTE_DATA_TYPES
             */
            DDCE_SWITCH: function(datum, datumType) {
                // Sets the game switch with id x to be on
                $gameSwitches.setValue(x, true);
                //
            }, // DDCE_SWITCH
             ...
             *                                      NOTE_DATA_TYPES
             */
            DDCE_VAR: function(datum, datumType) {
                // Sets the value of the game variable with id x to be y
                $gameVariables.setValue(x, y);
                //
            }, // DDCE_VAR
