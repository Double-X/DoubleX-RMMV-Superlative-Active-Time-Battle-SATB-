...
DoubleX_RMMV["Superlative ATB Configurations"] = "v0.04a";
//
...
(function(SATB) {
    ...
    SATB.params = {
        ...
        bar: {
            ...
            atbBarText: function() {
                ...
                //
                // Checks if the battler's not charging an action to be executed
                if (!this.isSATBCharge()) {
                    // The current ATB value
                    var cur = Math.floor(this.coreSATB());
                    //
                    // The maximum ATB value
                    var max = Math.floor(this.coreMaxSATB());
                    //
                ...
                //
                // The current charge ATB value
                var cur = Math.floor(this.chargeSATB());
                //
                // The maximum charge ATB value
                var max = Math.floor(this.chargeMaxSATB());
                //
                // The name of the currently charging skill/item
                var itemName = this.latestSATBItem_.item.name;
                // cur ATB value/max ATB value : virtual act slot num item name
            ...
            atbBarXOffset: function() {
                // Returns -80 and -40 if the battler's charging and not so
                return this.isSATBCharge() ? -80 : -40;
                //
            }, // atbBarXOffset
             ...
             */
            atbBarW: function() {
                // Returns 160 and 80 if the battler's charging and not charging
                return this.isSATBCharge() ? 160 : 80;
                //
            }, // atbBarW
        ...
        charge: {
            ...
            IsChargeEnabled: function() {
                return false; // Always disable the Charge Module
            }, // IsChargeEnabled
            ...
            isPayBeforeExecCharge: function() {
                // Always pay the skill/item cost right after executing them
                return false;
                //
            ...
            canCancelCharge: function() {
                // Always let players cancel the battler skill/item ATB charge
                return true;
                //
            }, // canCancelCharge
            ...
            canForceCharge: function() {
                // Always let players force the battler skill/item ATB charge
                return true;
                //
            } // canForceCharge
    ...
    SATB.notes = {
         ...
         *    - Setups IBVX used by this plugin's coreActState notetags
         *--------------------------------------------------------------------*/
        ...
        /*--------------------------------------------------------------------
         *    Charge ATB Max Functions
         *    - Setups CHMATBX used by this plugin's coreMax notetags
         *--------------------------------------------------------------------*/
         ...
         */
          chargeMax: {
             ...
             * @enum @param {DatumType} datumType - Refers to reference tag
             *                                      NOTE_DATA_TYPES
             * @param {+ve Number} latestChargeMax - Max charge ATB value up to
             *                                       latest note
             *                                       DON'T USE IT IF THE
             *                                       CHAINING RULE IS AN
             ...
             *                                       Refer to reference tag
             *                                       ASSOCIATIVE_CHAINING_RULE
             * @returns {+ve Number} The maximum charge ATB value(Which should
             *                       be positive) for the battler involved
             */
        ...
        isPayBeforeExecCharge: {
            ...
            IPBEC_TRUE: function(datum, datumType) {
                // Always pay the skill/item cost upon finishing inputting it
                return true;
                //
            }, // IPBEC_TRUE
            ...
            IPBEC_BATTLER_NAME: function(datum, datumType) {
                // Pay skill/item cost upon inputting if battler name is Test
                return this.name() === "Test";
                //
            }, // IPBEC_BATTLER_NAME
            ...
            IPBEC_SWITCH: function(datum, datumType) {
                // Pay skill/item cost upon inputting if switch with id x is on
                return $gameSwitches.value(x);
                //
            }, // IPBEC_SWITCH
        ...
        canCancelCharge: {
            ...
            CCC_TRUE: function(datum, datumType) {
                // Always lets players cancel the ATB charge
                return true;
                //
            }, // CCC_TRUE
         ...
         *    - Setups CFCX used by plugin's canCancelCharge notetags
         *--------------------------------------------------------------------*/
        ...
        canForceCharge: {
            ...
            CFC_TRUE: function(datum, datumType) {
                // Always lets players force the ATB charge
                return true;
                //
            }, // CFC_TRUE
