...
DoubleX_RMMV["Superlative ATB Compatibilities"] = "v0.06a";
//
 ...
 *      12. (v0.05b+) The visuals of action sequences of actor sprites will be
 ...
 *            action sequences
 *    # (v0.06a+)Yanfly Engine Plugins - Battle Engine Extension -
 *      Animated Sideview Enemies
 *      1. The battler sprite ATB bars are needlessly mirrored as well when
 *         mirroring animated sideview enemies
...
(function(SATBC) {
    ...
    _GB.requestMotionRefresh = $.requestMotionRefresh;
    _SATBC.requestMotionRefresh = $.requestMotionRefresh = function() {
    // v0.05b - v0.05b; Rewritten
        var isDead = this.isDead(), deadMotion = this.deadMotion();
        var isDeadMotion = this._motionType === deadMotion;
        if (isDead && !isDeadMotion) this.requestMotion(deadMotion);
        if (isDead && isDeadMotion || this._motionType === 'victory') return;
        var isInputting = BattleManager.isInputting();
        if (!isInputting && this._motionType === 'escape') return;
        if (!isInputting && this._motionType === 'guard') return;
        this.clearMotion();
        // Rewritten to stop refreshing motion when executing action sequences
        if (_SATBC._isRefreshMotion.call(this)) this.battler().refreshMotion();
     ...
     */
    _SATBC._isRefreshMotion = function() {
        if (!this.battler() || !BattleManager.isInputting()) return false;
        if (!SATBManager.isEnabled()) return true;
        var phase = BattleManager._phase;
        return phase !== "actionList" && phase !== "actionTargetList";
    }; // _SATBC._isRefreshMotion
 ...
 *      - Fixes the mirroring of ATB bars of animated sideview enemie sprites
 *----------------------------------------------------------------------------*/
...
(function(SATBC) {
    ...
    SATBC.Window_SATBBar = { orig: {}, new: {} };
    var $ = Window_SATBBar.prototype, $$ = Window_SATBBarBase.prototype;
    ...
    $._updateVisibleWin = function() {
        $$._updateVisibleWin.call(this);
        // Added to mirror the ATB bar again if its parent's mirrored
        if (!this.parent) return; // The bar only has parent after initialize
